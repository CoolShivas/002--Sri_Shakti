...
const MenUniformsPage = () => {
  ...
  // 🔹 Extract unique subtypes dynamically for "Mens"
  const uniqueSubtypes = useMemo(() => {
    const mensUniforms = allUniforms.filter(
      (u: any) => u.uniformType === "Mens"
    );
    const seen = new Set<string>();
    return mensUniforms.filter((u: any) => {
      ...
      uniqueSubtypes
        .map((item: any) => {
          ...
          return {
            ...
            link: `/menuniform/${item.uniformSubtype
              .replace(/\s+/g, "")
              .toLowerCase()}`,
            data: uniformData[0],
          };
  ...
  const heroImageUrl =
    logoHeroArr.find(
      (item: any) =>
        item?.name === "men-hero-section" && item?.type === "background"
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        ...
        <div className="container mx-auto text-center relative z-10 px-4">
          ...
          >
            Browse our men uniform categories.
          </motion.p>
        </div>
      </section>
...
export default MenUniformsPage;
