...
/**
 * Select by type (case-insensitive; STRICT match only)
 */
export const selectUniformsByType = createSelector(
  [selectAllUniforms, (state, type) => type],
  (uniformArr, type) => {
    ...
    const result = uniformArr.filter((u) => {
      const ut = norm(u.uniformType ?? u.type);
      return ut && ut === t; // strict type match only
    });
    console.log(`selectUniformsByType(${type}) result:`, result); // Debug: Log result
    return result;
  }
...
/**
 * Select by type + optional subtype (STRICT match only).
 * - If subtype missing in API -> only type match is applied.
 */
export const selectUniformsByTypeAndSubtype = createSelector(
  [selectAllUniforms, (state, type, subtype) => ({ type, subtype })],
  (uniformArr, { type, subtype }) => {
    ...
    const result = uniformArr.filter((u) => {
      const ut = norm(u.uniformType ?? u.type);
      if (!ut || ut !== t) return false; // strict type check

      // no subtype requested OR API doesn’t provide subtype → type match is enough
      if (!st || !u.uniformSubtype) return true;

      const uSub = norm(
        u.uniformSubtype ?? u.subtype ?? u.subType ?? u.categorySubtype
      );

      // subtype must match strictly
      return uSub === st;
    });
